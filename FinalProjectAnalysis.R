...
diabetes_train %>%
  ...
  filter(diabetes_binary == "No") %>%
  summary(count = n(diabetes_binary))
...
boost_params2 <- parameters(boost_tree_spec) %>%
  update(mtry = mtry(c(1, 20))) %>%
  update(learn_rate = learn_rate(c(-5, -0.2)))
boost_grid2 <- grid_regular(boost_params2, levels = 5)
...
## Tuning (just fitting for logistic regression)
# random forest
...
## Select Model
# Boosted tree from the third recipe had the best fold with a mean accuracy of 0.751
# and lowesr standard error than any other models with the same mean accuracy.
# The best random forest model has `mtry=11`, `min_n=30`, and `learn_rate=0.631`
